...
class DatabasesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create database" do
    assert_difference('Database.count') do
      post databases_url, params: { database: { name: @database.name, password: @database.password, port: @database.port, type: @database.type, url: @database.url, username: @database.username } }
    end
  ...
  test "should update database" do
    patch database_url(@database), params: { database: { name: @database.name, password: @database.password, port: @database.port, type: @database.type, url: @database.url, username: @database.username } }
